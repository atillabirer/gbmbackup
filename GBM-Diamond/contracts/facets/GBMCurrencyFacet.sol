...
/// @title GBMCurrencyFacet Contract
/// @author Guillaume Gonnaud
contract GBMCurrencyFacet is IGBMEventsFacet, IGBMCurrencyFacet {
    ...
    function setCurrencyName(uint256 currencyIndex, string calldata currencyName) external onlyAdmin {
        ...
        emit Currency_NameUpdated(currencyIndex, currencyName);
    }
    ...
    function setCurrencyAddress(uint256 currencyIndex, address currencyAddress) external onlyAdmin{
        ...
        emit Currency_AddressUpdated(currencyIndex, currencyAddress);
    ...
    /// @param currencyAddress The address of the currency. If set to 0x0, it mean that the 
    /// @param currencyName The new name of the currency
    function setCurrencyAddressAndName(uint256 currencyIndex, address currencyAddress,string calldata currencyName) external onlyAdmin {
        require(currencyIndex != 0, "currencyIndex cannot be equal to 0");
        s.currencyAddress[currencyIndex] = currencyAddress;
        s.currencyNames[currencyIndex] = currencyName;
        emit Currency_AddressUpdated(currencyIndex, currencyAddress);
    ...
    function setDefaultCurrency(uint256 currencyIndex) external onlyAdmin{
        ...
        emit Currency_DefaultUpdated(currencyIndex,s.currencyAddress[currencyIndex], s.currencyNames[currencyIndex]);
        
    }

    /// @notice Get the index of currency to be used by default if no currency params are specified
    /// @return currencyIndex The index of the default currency
    function getDefaultCurrency() external view returns (uint256){
