...
function initPage() {
    ...
    document.getElementById("token-metadata").value = JSON.stringify(newMetadata, null, '\t');
    document.getElementById("smallest-bundle").onchange = function (event) { document.getElementById("smallest-bundle-display").value = event.target.value }
    document.getElementById("biggest-bundle").onchange = function (event) { document.getElementById("biggest-bundle-display").value = event.target.value }
    document.getElementById("whale-factor").onchange = function (event) { document.getElementById("whale-factor-display").value = event.target.value }
    
    Array.from(document.getElementsByClassName("metadata-trigger")).forEach((_element) => {
        _element.value = "";
...
function downloadMetadataJSON() {
    ...
    var data_string = 'data:text/json;charset=utf-8,' + encodeURIComponent(document.getElementById("token-metadata").value);
