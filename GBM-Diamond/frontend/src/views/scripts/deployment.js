...
async function initDeploymentPage() {
    ...
    document.getElementById('deployer-address').placeholder = window.ethereum.selectedAddress ?? 'Please connect to your MetaMask account';
    ...
    document.getElementById('admin-address').value = window.ethereum.selectedAddress ?? 'Please connect to your MetaMask account';
    ...
    document.getElementById('enable-customization').onchange = enableCustomization;

    // Set default preset values
    defaultPresets = await (await fetch('/presets')).json();
    document.getElementById('preset-name').value = defaultPresets.GBMPresetArray[0].name;
    document.getElementById('preset-auction-duration').value = defaultPresets.GBMPresetArray[0].auctionDuration;
    document.getElementById('preset-cancellation').value = defaultPresets.GBMPresetArray[0].cancellationPeriodDuration;
    document.getElementById('preset-hammer').value = defaultPresets.GBMPresetArray[0].hammerTimeDuration;
    document.getElementById('preset-incentive-mul').value = defaultPresets.GBMPresetArray[0].incentiveGrowthMultiplier;
    document.getElementById('preset-incentive-max').value = defaultPresets.GBMPresetArray[0].incentiveMax;
    document.getElementById('preset-incentive-min').value = defaultPresets.GBMPresetArray[0].incentiveMin;
    document.getElementById('preset-step-min').value = defaultPresets.GBMPresetArray[0].stepMin;
}
...
function enableCustomization(_onChangeValue) {
    document.getElementById("default-preset-container").hidden = !_onChangeValue.target.checked
}
...
function connectToDeployer() {
    ...
    webSocket.onopen = (event) => {
        webSocket.send(`PRE-DEPLOYMENT || ${window.ethereum.selectedAddress} || ${JSON.stringify(defaultPresets)}`);
    }
    ...
    webSocket.onmessage = (event) => {
        ...
        switch (commands[1]) {
            case 'MSG': 
                ...
                document.getElementsByClassName('configuration-terminal-text')[0].innerHTML += msgToDisplay + '<br/>';
                break;
            case 'TST':
                document.getElementsByClassName('configuration-terminal-text')[0].innerHTML += 'All done!<br/>';
                step++;
    ...
    switch(commands[1]) {
        ...
        case "TST":
            const deploymentDetails = {
                network: document.getElementById("select-network").value,
